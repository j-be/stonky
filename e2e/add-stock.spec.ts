...
test('add ESPPs', async ({ page }) => {
	...
	await expect(await overview.nth(1).textContent()).toBe('Price paid 9,889.92 € (10,988.80 $)');
	await expect(await overview.nth(2).textContent()).toBe('Net if sold now 10,626.97 € (11,807.74 $)');
	await expect(await overview.nth(3).textContent()).toBe('Net profit if kept 1,630.08 € (1,811.20 $)');
	await expect(await overview.nth(4).textContent()).toBe('Net profit if sold now 737.05 € (818.94 $)');

	const details = espp.getByRole('table').nth(1).getByRole('row');
	await expect(await details.nth(1).textContent()).toBe('Current value 11,520.00 € (12,800.00 $)');
